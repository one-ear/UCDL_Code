...
def get_parser():
    ...
    parser.add_argument(
        '--config',
        default='1740037659899/code/BPM-GCN-master/config/EGait_journal/train_diff_combine_double_score_fagg.yaml',
    ...
    # parser.add_argument(
    #     '--weights',
    #     default='BMP-GCN-master_result/egait_runs/runs_diff_combine_fagg/fagg_att_cascade_st/best_acc.pt',
...
class Processor():
    ...
    def load_model(self):
        ...
        self.graphContrast = InfoNCEGraph(in_channels=3*16*16, out_channels=256, class_num=self.arg.model_args["num_class"], \
    ...
    def train(self, epoch, save_model=False):
        ...
        for batch_idx, (data_m,data_p, label,feature, index) in enumerate(process):
            ...
            # loss = loss1 + loss2
            output_p,output2,output_m,graph  = self.model(data_p,data_m)
            # output_p,output2,graph  = self.model(data_p,data_m)
            
            # ts_p,output_p, output2,graph = self.model(data_p,data_m)
            # output=output_p
            # print(graph.shape)
            ...
            # labels = torch.randint(0, 4, (len(output),)).cuda()
            # a = F.softmax(output, dim=1)
            output_p2 = F.softmax(output, dim=1)
            mask = torch.zeros_like( output_p2)
            b=label.unsqueeze(1)
            mask=mask.scatter_(1,b,torch.ones_like(b).float())
            p=output_p2[mask.bool()]
            beta=0.9  ###=0
            gamma=1  ###=1
            # loss_target = beta * ((1-p).pow(gamma) * self.loss(output, label)).mean()
            loss_target =beta* ((1-p).pow(gamma) * self.loss(output_p2, label)).mean()

            loss1_m=self.loss(output_m,label)
            loss1_p = self.loss(output_p, label)
            loss_combine=self.loss(output,label)
            loss2=self.loss2(output2,feature)
            loss =loss1_m+loss1_p +loss2+loss_combine+loss_target
            # loss =loss1_m+loss1_p +loss2+loss_combine
            
            # loss =loss1_p +loss2+loss_combine
            ...
            self.train_writer.add_scalar('loss_combine', loss_combine, self.global_step)
            self.train_writer.add_scalar('loss_target', loss_target, self.global_step)
            self.train_writer.add_scalar('contrast_loss', contrast_loss, self.global_step)
    ...
    def eval(self, epoch, save_score=True, loader_name=['test'], wrong_file='1740037659899/code/BPM-GCN-master/wrong.txt', result_file='1740037659899/code/BPM-GCN-master/result.txt'):
        ...
        for ln in loader_name:
            ...
            for batch_idx, (data_m,data_p, label,feature, index) in enumerate(process):
                ...
                with torch.no_grad():
                    ...
                    output_p, output2,output_m,graph = self.model(data_p,data_m)
                    # ts_p,output_p, output2 = self.model(data_p,data_m)
                    # ts_p,output_p, output2,graph = self.model(data_p,data_m)
                    # output=output_p
                    
#                     ts_p,output_p, output2,output_m,graph = self.model(data_p,data_m)
#                     print(type(ts_p))
#                     print(ts_p.shape)
#                     np.save(self.arg.model_saved_name + '-' + str(batch_idx) + '-' + str(int(self.global_step)) + '.npy',np.array(ts_p.cpu()))
#                     data_print = np.load(self.arg.model_saved_name + '-' + str(epoch) + '-' + str(int(self.global_step)) + '.npy')
#                     print(data_print)
