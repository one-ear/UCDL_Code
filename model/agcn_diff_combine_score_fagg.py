...
class Model(nn.Module):
    def __init__(self, num_class=4, num_point=16, num_constraints=31, graph=None, graph_args=dict(), in_channels_p=3,in_channels_m=8):
        ...
        self.fc1_classifier_total = nn.Linear(512, num_class)
    ...
    def forward(self, x_p, x_m):
        ...
        x_m,graph2 = self.l10_m(x_m)
   
        x_p,x_m=self.fusion3(x_p,x_m)

        # graph = torch.cat([graph2, graph], -1)
        graph = graph2*graph
        # N*M,C,T,V
        c_new_m = x_m.size(1)
        ...
        return self.fc1_classifier_p(x_p),self.fc2_aff(x_p),self.fc1_classifier_m(x_m), graph##如果要用t-sne可视化，返回的是未fc之前的特征
        # return x_mix,self.fc1_classifier_p(x_mix),self.fc2_aff(x_p), graph
        # return self.fc1_classifier_total(x_cat),self.fc2_aff(x_p), graph


class Model_Single(nn.Module):
    def __init__(self, num_class=4, num_point=16, num_person=1, num_constraints=31,graph=None, graph_args=dict(), in_channels_p=3, in_channels_m=8):
    ...
    def forward(self, x_p,x_m):
        ...
        return self.fc1_classifier(x), self.fc2_aff(x), graph
